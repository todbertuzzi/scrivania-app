...
function ensureRootElement() {
  ...
  if (!container) {
    ...
    // Aggiungi al DOM
    const targetContainer = document.querySelector('.container') || document.querySelector('main') || document.body
...
function attemptMount(retries = 0) {
  ...
  try {
    ...
    console.warn(`Tentativo ${retries + 1} fallito, riprova in ${RETRY_DELAY}ms...`, error)
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
  ...
  document.addEventListener('DOMContentLoaded', () => setTimeout(attemptMount, 100))
}
